...
async def lifespan(app: FastAPI):
    global model, scaler, encoders, fe_params, feature_names
    try:
        model = joblib.load('artifacts/model.pkl')
        scaler = joblib.load('artifacts/scaler.pkl')
        encoders = joblib.load('artifacts/encoders.pkl')
        fe_params = joblib.load('artifacts/fe_params.pkl')
...
async def predict(request: PredictRequest):
    try:
        ...
        # Apply feature engineering — must match src/feature_engineering.py exactly
        ...
        input_data['high_value_customer'] = (input_data['monthly_spend'] > fe_params['spend_75th']).astype(int)
